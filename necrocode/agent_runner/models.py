...
class TaskContext:
    ...
    # LLM settings
    related_files: Optional[List[str]] = None  # Files to include in LLM context
    max_tokens: Optional[int] = None  # Maximum tokens for LLM generation
...
class ImplementationResult:
    ...
    llm_model: Optional[str] = None  # LLM model used for code generation
    tokens_used: Optional[int] = None  # Total tokens used in LLM generation
...
class SlotAllocation:
    """
    Slot allocation information from Repo Pool Manager.
    
    Contains the slot ID and path allocated for task execution.
    """
    slot_id: str
    slot_path: Path
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "SlotAllocation":
        """Create from dictionary."""
        data = data.copy()
        data["slot_path"] = Path(data["slot_path"])
        return cls(**data)
...
class CodeChange:
    """
    Code change generated by LLM.
    
    Represents a single file operation (create, modify, or delete)
...
class LLMResponse:
    ...
    Contains the code changes generated by the LLM, along with
    metadata about the model used and tokens consumed.
    """
...
class LLMConfig:
    ...
    Contains API credentials, model selection, timeout settings,
    and token limits for communicating with LLM services like OpenAI.
    """
    api_key: str
    model: str = "gpt-4"
